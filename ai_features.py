...
try:
    ...
    AI_AVAILABLE = False
    print("AI features not available. Install requirements: pip install -r requirements.txt")
...
class AIMessageAssistant:
    ...
    async def generate_message_suggestions(self, context: str, tone: str = "professional") -> List[str]:
        """Generate AI-powered message suggestions"""
        if not self.openai_client:
            return ["AI features not configured. Please set OPENAI_API_KEY in .env file."]
...
class SmartScheduler:
    ...
    def log_message_analytics(self, phone_number: str, send_time: datetime, 
...
class ModernAnalytics:
    ...
    def generate_delivery_report(self) -> Dict:
        ...
        return {
            ...
            "best_hours": [{"hour": h[0], "success_rate": round(h[2] * 100, 2)} 
                          for h in peak_hours[:3]] if peak_hours else []
        }
